...
if (isset($_POST['acct-ad'])) {
    if (isset($_GET['id'])) {
        $ad_id = intval($_GET['id']);
        $query_accept = "UPDATE ads SET status = 1 WHERE ad_id = '$ad_id'";
        $result_accept = mysqli_query($con, $query_accept);

        if ($result_accept) {
            echo '<script>alert("Ad accepted!")</script>';
...
if (isset($_POST['del-ad'])) {
    if (isset($_GET['id'])) {
        $ad_id = intval($_GET['id']);
        $query_delete = "DELETE FROM ads WHERE ad_id = '$ad_id'";
...
if (isset($_GET['id'])) {
    ...
    if ($ad_details = $result_details->fetch_assoc()) {
        ...
        $ad_condit = ($ad_condit == 1) ? "New" : "Used";
...
<a href="adm.php"><img src="../resources/back.png" style="width: 20px; height: 20px; margin-top: 10px; margin-left: 20px;"></a>
<main>
<div id="ad-container">
            ...
            <div id="ad-details">
                ...
                <p><strong>Publish by:</strong> <?php echo htmlspecialchars($owner_name); ?></p>
            </div>
        </div>
 <form method="post" action="" name="form-ad" class="ad-form">
            <input type="hidden" name="ad-id" value="<?php echo htmlspecialchars($ad_id); ?>">
            <div class="form-buttons">
                <button type="submit" class="search-button" name="acct-ad">
                    <img src="../resources/accept.png" alt="Search Icon" class="search-icon">
                </button>
                <button type="submit" class="search-button" name="del-ad">
                    <img src="../resources/delete.png" alt="Search Icon" class="search-icon">
                </button>
            </div>
        </form>
</main>
