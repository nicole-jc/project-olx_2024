...
<!DOCTYPE html>
<head>
    <title><?php echo htmlspecialchars($_SESSION["user-name"]);?>'s bag</title>
...
<div class="main-container">
    ...
    <?php
        ...
        $query_search = "SELECT a.* FROM bag b INNER JOIN ads a ON b.ad_id = a.ad_id WHERE b.user_id = '$user_logged'";
        ...
        if (isset($_GET['search'])) {
            ...
            // SQL consult to search by title only in wishlist items
            $sql_code = "SELECT a.* FROM wishlist w INNER JOIN ads a ON w.ad_id = a.ad_id WHERE w.user_id = '$user_logged' AND a.adtitle LIKE '%$searching%'";
        ...
        if (mysqli_num_rows($result_search) > 0) {
            while ($row_ad = mysqli_fetch_assoc($result_search)) {
                ...
                echo '<img src="../' . $row_ad['ad_image_path'] . '" alt="' . $row_ad['adtitle'] . '">';
                echo '<div class="desc">' . $row_ad['adtitle'] . '</div>';
                echo '</div>';
                echo '<h4>' . $row_ad['adtitle'] . '</h4>';
                echo '<button type="submit" class="search-button" name="buy-btn">Buy</button>';
                echo '</div></a>'; 
            }
        } else {
            echo '<p style="color: white;">Your bag is empty .</p>';
        }
    ?>
</div>
</div>
